    ...
    <div className='flex items-center justify-between py-5 font-medium'>
      ...
      <img  onClick={() => setShowSearch(true)} src={assets.search_icon} alt="" className='w-5 cursor-pointer' />
           ...
           <Link to='/cart' className='relative'>
            ...
            <p className='absolute right-[-5px] top-[-10px] w-4 text-center leading-4 bg-black text-white aspect-square rounded-full text-[8px]'>{getCartCount()}</p>
            </Link>
      ...
      <div className={`absolute top-0 right-0 bottom-0 overflow-hidden bg-white transition-all ${visible ? 'w-full' : 'w-0' }` }>
        <div className='flex flex-col text-gray-600'>
            ...
            <hr />
            <NavLink onClick={() => setVisible(false)}  className='py-2 pl-6 '  to='/abut'>About</NavLink>
            <hr />
